...
function interfaceInMove(movetoHyViewer) {
  ...
  if (ClientLastMadeMove==movetoHyViewer.substring(0,movetoHyViewer.indexOf('.')) )
  {  // We already made move on our client board
...
var ord;
  ...
  {
    pos=bpgntext.indexOf("[Event"); ord=1;
...
{var i; var piece;
 ...
 { if (this.pos[i].charAt(0)!=op) {continue;};
   ...
   { //alert('checked '+whomove+' '+piece.toLowerCase() +' from '+i+' to '+dest)
    return true;
...
{var i; var piece; var numberofchecks=0;
 op=othercolor(whomove);
 for (i=0;i<64;i++) {
 if (this.pos[i].charAt(0)!=op) {continue;};
 piece=this.pos[i].charAt(1);
...
{ //return if supposed move is not out of board
  ...
  var sfile=getfile(sqsrc); var srank=getrank(sqsrc); //rank return 1..8
  var dfile=getfile(sqdest); var drank=getrank(sqdest); //file return 0..7
...
var r; var f; var c; var f; var c2; var src=-1; var king; var dp; var sp; var ep; var eptakingsq=0;
var quit=false;
...
if (!this.legalmove(piece,whomove,src,dest,0)) {continue;} 
 ...
 //make move trick
   this.pos[src]=''; this.pos[dest]=whomove+piece;
 ...
 {eptakingsq=this.enpasssq+((getrank(this.enpasssq)==3)?(-8):8); ep=this.pos[eptakingsq]; this.pos[eptakingsq]='';
 ...
 if (!this.incheck(whomove,king)) 
   {found=true;quit=true;};

 this.pos[src]=sp; this.pos[dest]=dp;
 if (eptakingsq!=0) {this.pos[eptakingsq]=ep;eptakingsq=0};
};
if (quit==true) break;
}; if (quit==true) {break;};}
if (!found) return -1;
...
//Math.trunc() function returns the integer part of a number by removing any fractional digits.
 ...
 var days=Math.floor(num/86400); num -=days*86400;
 var hours=Math.floor(num/3600); num -=hours*3600; 
 var minutes=Math.floor(num/60); num -=minutes*60;
 var secs=Math.floor(num); num -=secs;
 var tenthofsecs=Math.floor(num*10);
 
 var time='';
 if (days>0) {time=''+days+' '+fixnumber(hours)+':'+fixnumber(minutes)+':'+fixnumber(secs);}
 else if (hours>0) {time=''+hours+':'+fixnumber(minutes)+':'+fixnumber(secs);}
 else {time=''+minutes+':'+fixnumber(secs);}
...
{var res="";
 if (fsPiece=='') {res+=(fiSquareIsBlack)?'+':' '}
 else if (fsPiece=='wp') {res+=(fiSquareIsBlack)?'P':'p'}
 else if (fsPiece=='wr') {res+=(fiSquareIsBlack)?'R':'r'}
 else if (fsPiece=='wn') {res+=(fiSquareIsBlack)?'N':'n'}
 else if (fsPiece=='wb') {res+=(fiSquareIsBlack)?'B':'b'}
 else if (fsPiece=='wq') {res+=(fiSquareIsBlack)?'Q':'q'}
 else if (fsPiece=='wk') {res+=(fiSquareIsBlack)?'K':'k'}
 else if (fsPiece=='bp') {res+=(fiSquareIsBlack)?'O':'o'}
 else if (fsPiece=='br') {res+=(fiSquareIsBlack)?'T':'t'}
 else if (fsPiece=='bn') {res+=(fiSquareIsBlack)?'M':'m'}
 else if (fsPiece=='bb') {res+=(fiSquareIsBlack)?'V':'v'}
 else if (fsPiece=='bq') {res+=(fiSquareIsBlack)?'W':'w'}
 else if (fsPiece=='bk') {res+=(fiSquareIsBlack)?'L':'l'}
 else {res+=' ';};
 return res;
}
...
var i,n; var t; var ec=0; var res=''; var col=0,row=0; var p; var lsTmp,lsTmp2;
var lsHoldBlack='',lsHoldWhite='';

lsTmp=holdText(this); n=lsTmp.length; i=0;
while (i<n)
{ lsTmp2=lsTmp[i].toLowerCase();
  if (lsTmp[i]==lsTmp2)
  {lsHoldBlack+=PiecesToRTF('b'+lsTmp2,0)}
  else {lsHoldWhite+=PiecesToRTF('w'+lsTmp2,0)}
  i++;
} 
...
if (this.turn==this.turn.toLowerCase()) {res+='>'} else {res+=' '};
res+='!""""""""#\r\n';res+=' $';
for (i=0;i<64;i++)
{
 t=this.pos[i];
...
if (this.turn==this.turn.toUpperCase()) {res+='>'} else {res+=' '};
res+='/(((((((()\r\n';
...
{var res='';
 ...
 var lnA=lsArrA.length;  var lnB=lsArrB.length; var ln;
 ln=lnA;
 if (lnA<lnB) {ln=lnB; while(lnA<ln){lsArrA[lnA]='\r\n'; lnA++;}}
 else if (lnB<lnA) {while(lnB<ln){lsArrB[lnB]='\r\n'; lnB++;}};
 var li1=0;
 while (li1<ln)
...
function NODE()
{
 ...
 this.cLag=''; this.cLagCompensation=''; //SKAcz Lag and Lag compensation
}
...
function tobpgn(mv) 
{ var bd=mv.board,wh=mv.side,p=mv.frompiece.charAt(1);
  var c='', fr=mv.fromsquare, prom='';  
...
if (this.gameevent=='') {this.gameevent="bughouse chess game"};
...
else {tmp=tmp+' <input class="normal" type="text" size="12" name="'+'upclock'+bd+'" onFocus="this.blur();"> '};
tmp=tmp+'</td>';
...
else {tmp=tmp+' <input type="text" size="16" name="up'+bd+'" onFocus="this.blur();"> '};
tmp=tmp+'</td>';
...
else {tmp=tmp+' <input class="normal" type="text" size="12" name="'+'dnclock'+bd+'" onFocus="this.blur();"> '} ;
tmp=tmp+'</td>';
...
if (this.displaymode!='diagram') {tmp+='<input disabled readonly type="text" size="12" name="lastmove'+bd+'" value=" " onChange="assexecmove(this.value,\''+bd+'\',\''+this.viewername+"'"+')"> ';};

if (this.reloadmode=="noreload") 
{tmp=tmp+tago+eval('this.'+dn)+'   '+this.rdo+eval('this.'+dn+'elo')+ this.rdc + tagc }
else {
 if (this.displaymode!="diagram") {tmp+=' <input type="text" size="16" name="dn'+bd+'" onFocus="assexecmove(document.'+this.viewername+'.lastmove'+bd+'.value,' +"'"+bd+"','"+this.viewername+"'"+'); this.blur();"> '}
 else {tmp+=' <input type="text" size="16" name="dn'+bd+'" onFocus="this.blur();"> '};
};
tmp=tmp+'</td>';
...
{var v=eval(viewer);
var t; var f=0; var mv; var moveind; var x; var tbd;
...
while (f<num) {
 ...
 if (!v.BPGN[moveind].dmove) 
 {
  ...
  if (move==false) {return}; //illegal move
...
{
 var t=this.turn;
 if ((t==t.toLowerCase() && step1>0)||(t!=t.toLowerCase() && step1<0)){this.movecount+=step1;}
 if (t==t.toLowerCase()) {this.turn=t.toUpperCase();} else {this.turn=t.toLowerCase();};
 this.brefreshform();
}
...
//Rotate layout gif1,2,  N,1  
      ...
      {lGifDirOrig="gif"+lGifNumber; lGifNumber++; lGifDir="gif"+lGifNumber;}
      else if (lGifNumber >= lGifNumberMax)
      {lGifDirOrig="gif"+lGifNumber; lGifNumber=1; lGifDir="gif"+lGifNumber;}    
      else
      {lGifNumber=1;  lGifDirOrig=lGifDir="gif"+lGifNumber;};    
...
//Change background squares
      ...
      for (i=0; i<8;i++)
      {
       ...
       { ind=8*i+j;
         lid="tdsquare"+"a"+ind; 
           lobj=document.getElementById(lid).style.backgroundImage;
           ltmp=lobj.replace(lGifDirOrig,lGifDir);ltmp=ltmp.replace(lGifDirOrig,lGifDir);
           document.getElementById(lid).style.backgroundImage=ltmp;
...
function TimeFormatToBpgnReplaceClkBySecs(fsBpgnGame)
{
 ...
 li1=0; li1=lsBody.indexOf('{[%clk',li1);
  while (li1> -1)
...
if (bpgntext.indexOf('[Setup "1"]')> -1)
 ...
 { var liBFEN1=bpgntext.indexOf('[FEN "')+6; var liBFEN2=bpgntext.indexOf('"]',liBFEN1);
...
function capture(bd,piece,num)
{
 ...
 if (this.capturemode=='atomic') {explosion(5)};
...
{
 if (ind-9>-1){this.pos[ind-9]=''};
 if (ind-8>-1){this.pos[ind-8]=''};
 if (ind-7>-1){this.pos[ind-7]=''};
 if (ind-1>-1){this.pos[ind-1]=''};
 if (ind+1<65){this.pos[ind+1]=''};
 if (ind+7<65){this.pos[ind+7]=''};
 if (ind+8<65){this.pos[ind+8]=''};
 if (ind+9<65){this.pos[ind+9]=''};
}
...
if ((l>0)&&(isnum(t[l-1]))) {this.bclock=Number(t[l-1])}; //202104 Number by SKAcz
if ((l>1)&&(isnum(t[l-2]))) {this.wclock=Number(t[l-2])};
